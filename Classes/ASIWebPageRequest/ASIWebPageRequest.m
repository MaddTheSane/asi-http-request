...
@property (strong, nonatomic) ASINetworkQueue *externalResourceQueue;
@property (strong, nonatomic) NSMutableDictionary *resourceList;
@end
...
- (void)finishedFetchingExternalResources:(ASINetworkQueue *)queue
{
	...
	if (![self parentRequest]) {
		...
		[[self class] updateProgressIndicator:&tmpDownloadDelegate withProgress:contentLength ofTotal:contentLength];
		downloadProgressDelegate = tmpDownloadDelegate;
	}
